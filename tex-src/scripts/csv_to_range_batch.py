...
def make_summary(rows: list[tuple[str, float, float, float]]) -> str:
...
def main() -> None:
    p = argparse.ArgumentParser(description="range batch processor")
    p.add_argument("--eta", type=float, default=ETA, help="学習率 η")
    p.add_argument("--init-lambda", type=float, default=L_INIT, help="初期 λ_vol")
    p.add_argument("--min-lambda", type=float, default=L_MIN, help="最小 λ_vol")
    p.add_argument("--max-lambda", type=float, default=L_MAX, help="最大 λ_vol")
    args = p.parse_args()
    ...
    for csv_path in sorted(PRICES_DIR.glob("*.csv")):
        ...
        rows.append((csv_path.stem, mae, rmae, hit))
